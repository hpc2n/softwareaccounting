...
pg3 = parser.add_argument_group("General aggregation options (i.e. do not list versions, accounts or users for this" + " software)")
...
pg4.add_argument("-l", "--lower", type=float, default=0.01, help="Don't list software with lower core-h usage than this (default 0.01)")
...
def print_usage_lines(items: defaultdict, total: float, levels: int, indent: int = 1) -> None:
    for name, val in sorted(items.items(), key=lambda v: v[1]["usage"], reverse=True):
        ...
        if total < 1e-3:
            return
        print(
            (" " * 3 * indent + "{:25}  {:5.1f}%" + " " * (42 + 3 * levels) + "{:4}").format(
...
print("Including jobs in the interval {} - {}".format(datetime.fromtimestamp(start), datetime.fromtimestamp(stop)))
print()
...
for name, sw in sorted(usage.items(), key=lambda v: v[1]["no_jobs" if args.jobs else "total"], reverse=True):
    ...
    if not name:
        name = "-"
    print(
        ("{:25}" + " " * (14 + all_indent) + "{:8}    {:5.1f}% {:7} {:5.1f}%     {:4}").format(
            name, int(total), 100 * total / grand_total, jobs, 100 * jobs / grand_total_jobs, round(sw["wsum"] / sw["time"])
        )
    )
